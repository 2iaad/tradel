// Shared Carbon Terminal class strings (auth page + dashboard modal).
export const inputCls =
    'w-full box-border bg-[#0a0d0f] border border-[#222a2f] rounded-lg px-3.5 py-[13px] text-[#e9eef0] text-sm outline-none transition-[border-color,box-shadow] duration-200 focus:border-[#2fd57f] focus:shadow-[0_0_0_3px_rgba(47,213,127,0.13)]';
export const labelCls =
    'block font-mono text-[10.5px] font-medium tracking-[0.14em] uppercase text-[#5f6b70] mb-[7px]';
export const btnCls =
    'w-full border-none rounded-lg p-3.5 mt-1 bg-[#2fd57f] text-[#04130a] font-semibold text-[14.5px] cursor-pointer transition-[background,transform] hover:bg-[#4ce392] active:scale-[0.985] disabled:opacity-60 disabled:cursor-default';
export const linkCls =
    'bg-transparent border-none p-0 text-[#2fd57f] font-medium cursor-pointer hover:text-[#5fe9a0]';
export const kickerCls = 'font-mono text-[11px] font-medium tracking-[0.2em] text-[#2fd57f]';
export const errorCls = 'm-0 text-[13px] text-[#f0554e]';

// Dashboard building blocks.
export const cardCls = 'bg-[#0e1214] border border-[#1b2226] rounded-[10px]';
export const h2Cls = 'm-0 text-[17px] font-semibold text-[#eef4f2]';
export const ctaCls =
    'border-none rounded-lg px-[18px] py-[11px] bg-[#2fd57f] text-[#04130a] font-semibold text-[13.5px] cursor-pointer transition-[background,transform] hover:bg-[#4ce392] active:scale-[0.97]';
export const ghostBtnCls =
    'bg-transparent border-none p-0 font-mono text-[11px] font-medium tracking-[0.1em] text-[#2fd57f] cursor-pointer hover:text-[#5fe9a0]';
