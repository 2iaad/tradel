    ...
    ),
    {
        variants: {
            variant: {
                ...
                ghost: 'visited:!text-foreground hover:!text-foreground focus-visible:!text-foreground active:!text-foreground text-foreground hover:bg-accent/60 dark:hover:bg-muted/50',
                destructive:
...
function resolvePrimitiveButtonProps(buttonProps: ButtonRenderProps) {
    /* eslint-disable @typescript-eslint/no-unused-vars -- Motion and Base UI expose
     overlapping DOM handlers; strip the primitive copies before forwarding. */
    const {
...
function FlowIdleLayer({
    ...
    idleLabel: string;
    idleOpacity: number;
    idleTransition: typeof FLUID_CONTENT | typeof FLUID_REVEAL | typeof NO_TRANSITION;
    isIdle: boolean;
    isShrinking: boolean;
}) {
    return (
...
function FlowIconSlot({
    ...
    return (
        <span className={cn('relative shrink-0 [&_svg]:size-[1em]', iconSlotClassName)}>
            <motion.span
                animate={{
...
function FlowContent({ animateEnabled, labels, phase, size, successIcon }: FlowContentProps) {
...
const FluxButton = React.forwardRef<HTMLButtonElement, FluxButtonProps>(
    (
        ...
        const widthAnimationRef = React.useRef<ReturnType<typeof animate> | null>(null);
        ...
        const { measureRef, widths } = useMeasuredWidths(layout.paddingX, layout.height);
        ...
        return (
            <>
                ...
                >
                    ...
                    <span
                        className={cn('inline-flex items-center', successIcon && busyGapClassName)}
                        data-width="success"
                    >
                        {successIcon ? (
                            <span className={cn('inline-flex shrink-0', iconSlotClassName)} />
