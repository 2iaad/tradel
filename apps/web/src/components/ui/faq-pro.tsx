...
const searchInputClassName = cn(
    ...
    'in-data-[size=compact]:h-10 in-data-[size=compact]:px-4 in-data-[size=compact]:pr-10 in-data-[size=compact]:text-[13px]',
...
const triggerClassName =
    'flex w-full items-start justify-between gap-4 px-5 py-4 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset disabled:cursor-not-allowed in-data-[size=compact]:px-4 in-data-[size=compact]:py-3';

const answerClassName =
    'px-5 pb-5 text-[14px] text-muted-foreground leading-6 in-data-[size=compact]:px-4 in-data-[size=compact]:pb-4 in-data-[size=compact]:text-[12px] in-data-[size=compact]:leading-5';
...
function itemSearchText(item: FaqProItem) {
    ...
    return [item.question, answerText, ...(item.keywords ?? [])].join(' ').toLowerCase();
}
...
function isOpenIdVisible(openId: string | null, visibleItems: FaqProItem[]) {
    return openId !== null && visibleItems.some((item) => item.id === openId && !item.disabled);
}
...
function sanitizePanelStyle(style?: React.CSSProperties) {
    ...
    /* eslint-disable @typescript-eslint/no-unused-vars -- Remove layout styles
     owned by Base UI before Motion animates the panel height. */
    const {
        height: _height,
...
function resolvePanelProps(panelProps: PanelRenderProps) {
    /* eslint-disable @typescript-eslint/no-unused-vars -- Base UI and Motion
     provide overlapping render props; strip the copies we do not forward. */
    const {
...
function FaqProPanelBody({ answer, isOpen, query, reduceMotion }: FaqProPanelBodyProps) {
    if (reduceMotion) {
        if (!isOpen) {
            return null;
        }
    ...
    return (
        ...
        >
            <AccordionPrimitive.Header className="flex">
                ...
                >
                    <span className="font-medium text-[15px] text-foreground leading-6 tracking-[-0.02em] in-data-[size=compact]:text-[13px] in-data-[size=compact]:leading-5">
...
const FaqPro = React.forwardRef<HTMLDivElement, FaqProProps>(
    (
        ...
        const [uncontrolledOpenId, setUncontrolledOpenId] = React.useState<string | null>(() =>
        ...
        const matchesQuery = React.useCallback(
            (item: FaqProItem, searchQuery: string) =>
                filter ? filter(item, searchQuery) : defaultMatchesQuery(item, searchQuery),
            [filter],
        );
        ...
        const setOpenId = React.useCallback(
            (next: string | null | ((current: string | null) => string | null)) => {
                if (isControlled) {
                    const resolved = typeof next === 'function' ? next(valueProp ?? null) : next;
        ...
        React.useEffect(() => {
            ...
            if (searchActive) {
                ...
                if (accordionOpenId !== openId) {
                    // Registry state sync: searching automatically opens the first match.
        ...
        return (
            ...
            >
                ...
                <output aria-live="polite" className="sr-only" data-slot="faq-pro-status">
                    {statusMessage}
                </output>
